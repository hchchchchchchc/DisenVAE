...
def serialize_user(pkl_name='pkl/user_ser_dict.pkl'):
    if os.path.exists(pkl_name) is True:
        pkl_file = open(pkl_name, "rb")
        data = pickle.load(pkl_file)
        return dict(data['user_ser_dict'])
    jsons = []
    for line in open("data/5-core.json").readlines():
    ...
    with open(pkl_name, "wb") as file:
        pickle.dump({'user_ser_dict': user_serialize_dict}, file)
    return user_serialize_dict


# 输入user和item的set，输出user和item从1到n有序的字典
def serialize_item(pkl_name='pkl/item_ser_dict.pkl'):
    if os.path.exists(pkl_name) is True:
        pkl_file = open(pkl_name, "rb")
        data = pickle.load(pkl_file)
        return dict(data['item_ser_dict'])
    jsons = []
    for line in open("data/5-core.json").readlines():
        jsons.append(json.loads(line))
    items = set()
    ...
    for item in items:
        ...
        item_idx += 1
    with open(pkl_name, "wb") as file:
        pickle.dump({'item_ser_dict': item_serialize_dict}, file)
...
# 新建一个user-item的交互字典
def build_user_item_interaction_dict(train_csv='data/train_set.csv',
...
# 新建一个item-user的交互字典
def build_item_user_interaction_dict(train_csv='data/train_set.csv',
...
class RatingDataset(torch.utils.data.Dataset):
    def __init__(self, train_csv, img_features, genres, category_num, user_serialize_dict,  positive_number, negative_number):
        ...
        self.item_serialize_dict = serialize_item()
        ...
        self.all_item_number = len(self.item_serialize_dict)
    ...
    def __getitem__(self, index):
        ...
        # 处理 item feature
        img_feature = self.img_feature_dict[self.item_serialize_dict.get(item)]
        ...
        return torch.tensor(user), torch.tensor(item), genres, torch.tensor(img_feature, dtype=torch.float32), torch.tensor(neg_user),\
