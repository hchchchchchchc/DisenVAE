...
class Validate:
    ...
    def start_validate(self, model):
        ...
        for it in self.item:
            ...
            image_feature = self.img_dict[item]
            image_feature = torch.tensor(image_feature, dtype=torch.float32)
            genres = genres.to(device)
            image_feature = image_feature.to(device)
            with torch.no_grad():
                recommend_users ,mean_a, mean_c, mean_p= get_similar_user_speed(model, item, genres, image_feature, max_k)
...
if __name__ == '__main__':
    ...
    # 参数解析器
    import pickle
    from support import RatingDataset, serialize_user, serialize_item
    ...
    train_path = "data/train_set_withneg.csv"
    vliad_path = 'data/test_set.csv'
    train_df = pd.read_csv(train_path)
    load_dir = '/root/data1/hc/CCFCRec/Amazon Sports/result/2025-01-20_13_26_56'
    # pkl_file = open(load_dir+'/save_dict.pkl', 'rb')
    image = get_img_feature_pickle()
    user_ser_dict = serialize_user()
    item_ser_dict = serialize_item()
    asin_category_int_map, category_ser_map = serial_asin_category()
    # data = pickle.load(pkl_file)
    dataSet = RatingDataset(train_df, image, asin_category_int_map, category_ser_map.__len__(),
    ...
    args.attr_num = len(category_ser_map)

    validator = Validate(validate_csv=vliad_path, user_serialize_dict=user_ser_dict, img=image, item_serialize_dict=item_ser_dict,
    ...
    for model in load_array:
        myModel.load_state_dict(torch.load(load_dir+'/'+model+'.pt', map_location=device))
